...
- [ ] **REDEPLOY `redeem-recovery-code` after the L610 fix**: `supabase functions deploy redeem-recovery-code --no-verify-jwt`. The function now returns HTTP 200 (was 4xx) for expected user failures (wrong code / locked) so `functions.invoke` surfaces the message instead of a generic "non-2xx" error. The happy path is unchanged, so live recovery already works; this only improves the wrong-code/locked message. Redeploy to ship it.
