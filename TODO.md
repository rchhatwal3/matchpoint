...
## Later
- [ ] **T9 — session TTL policy (research + decide)** — dispatch a research agent to survey best practices for session length on a passwordless, anonymous-first couples app: bounded session (time-boxed max length + inactivity timeout) vs. the current 1h-JWT / indefinite-refresh default. Cover Supabase's `Time-box user sessions` / `Inactivity timeout` / refresh-token rotation settings, the security-vs-friction tradeoff for a low-sensitivity social app, and mobile expectations (stay-logged-in like WhatsApp). Deliver a one-page summary + a recommended setting to `docs/`; the chosen policy is a Supabase dashboard toggle (→ MANUAL_TODOS), not app code. User leans "bounded is just as secure" — validate.
- [ ] **T9 — lost-email account recovery** — passwordless means a permanent email account is unrecoverable if the user loses email access AND has no live device session. Design + implement a fallback. Options to weigh: (a) one-time recovery codes shown at upgrade time; (b) a second linked email; (c) rely on Google/Apple as an alternate factor once those land — but that collides with the product rule "no cross-provider override," so needs an explicit carve-out; (d) accept the risk for MVP with a clear warning at upgrade ("this email is your only key"). Pick one, note dashboard/schema needs. Until built, add a warning on the /account upgrade screen.
