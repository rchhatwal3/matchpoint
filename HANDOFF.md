...
- **NEXT: T9 lost-email recovery Phase B (recovery codes).** Spec `docs/superpowers/specs/2026-07-23-t9-recovery-codes-design.md` — hashed `recovery_codes` table + two edge fns (`issue-recovery-codes` JWT-gated; `redeem-recovery-code` unauthenticated + rate-limited, mints a session via admin `generateLink`, no email) + client redeem flow + tests. Build on its own branch off `main`. Open questions need a call first: code count (8?), auto-prompt at upgrade vs. manual button, redeem lockout threshold.
