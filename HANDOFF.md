...
**NOTE (2026-08-03, later): the two P2s below are now FIXED — see the section at the top of this file.** They are left here with their original wording because the reasoning is still worth reading, and because the second one's diagnosis turned out to be wrong in an instructive way. Do not treat either as open work.

- ~~P2 — the `member_room_id` membership oracle~~ → **CLOSED** by PR #56 / migration `026`. Verified live by every route tried.
- ~~P2 — a location below `CACHE_TARGET` never caches~~ → **FIXED** by PR #57 / migration `027`. The diagnosis was wrong: New York was a *truncated fetch*, not a sparse location.


- **P2 — the `member_room_id` membership oracle. REPRODUCED LIVE 2026-08-03.** A fresh session that has never been a member of any room called `POST /rest/v1/rpc/member_room_id` with another user's member id and received that user's room UUID (200). `002_rls.sql:5-8` has no REVOKE and PostgreSQL grants EXECUTE to PUBLIC by default. A `REVOKE ... FROM public` is not enough — the attacker holds an anonymous session and therefore *is* `authenticated`. Fix is to move it to an unexposed schema (`private.member_room_id`), grant EXECUTE there, and re-point the five policies that call it (seven call sites — `rooms_update_members` calls it in both `USING` and `WITH CHECK`, `swipes_select_same_room` twice in one expression). Note the parameter is `p_member`, not `p_uid`.
