...
--
-- Section 2 is wrapped in an explicit BEGIN/COMMIT for the same reason 023:183
-- and 028:74-80 were: `swipes.item_id` and `matches.item_id` are ON DELETE
-- CASCADE (001:36, 021:38), so a DELETE of items that runs after a failed
-- repoint destroys real user swipes and the erasure snapshots 021 exists to
-- preserve — permanently and silently. Applying this file statement by
-- statement, which is what any tool that autocommits per statement does, is
-- exactly that scenario. Run section 2 as one transaction.
--
-- Section 1 is deliberately OUTSIDE that transaction and runs first. It touches
-- no user data, and its assertions abort the whole apply before section 2's
-- BEGIN is ever reached.
...
--
-- A real table rather than TEMP, and DROPped first, for 028:552-554's reason: an
-- apply that aborted partway leaves it behind, and every retry would then fail
-- here rather than at the statement that actually broke. It is dropped again at
-- the end of section 2.
DROP TABLE IF EXISTS bare_city_location_map;

CREATE TABLE bare_city_location_map (
...
-- abort here leaves nothing behind but the map table above, which the DROP at
-- the top of a retry removes.
--
...
-- ---------------------------------------------------------------------------
-- 2. THE CORRECTION — ONE TRANSACTION, see the header
-- ---------------------------------------------------------------------------
BEGIN;

-- Blocks concurrent writers to `items` for the rest of the transaction, for
-- 028:477-484's reason: the collision map in section 3 is a snapshot, and a live
...
--
-- `rooms` gets NO lock, again as 028:485. The rooms rewrite below is
...
DROP TABLE IF EXISTS bare_city_touched_buckets;

CREATE TABLE bare_city_touched_buckets AS
...
--
-- This is 028 section 5c's shape, reused rather than reinvented: same real
-- table, same DROP-first retry safety, same keeper preference, same
...
DROP TABLE IF EXISTS bare_city_dupe_map;

-- Keeper preference copied verbatim from 028:555-560 (itself from 023:206-216),
...
CREATE TABLE bare_city_dupe_map AS
...
--
-- The GROUP BY is load-bearing, exactly as in 028:572-577 and 023:228-234: ON
...
-- wrong for 028:628-631's reason: two markers can collide on the primary key,
...
DROP TABLE bare_city_touched_buckets;
DROP TABLE bare_city_location_map;
